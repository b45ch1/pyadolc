/*----------------------------------------------------------------------------
 ...
 File:     uni5_for.c
 Revision: $Id: uni5_for.c 278 2008-12-19 09:11:26Z awalther $
...
#define TAYLOR_BUFFER up_T
    ...
    temp_array = (locint *)  malloc(sizeof(locint) * (NUMNNZ));
    temp_array1 = (locint *)  malloc(sizeof(locint) * (NUMNNZ));
    max_arr_len = NUMNNZ;
    max_arr_len1 = NUMNNZ;
...
void merge_2_index_domains(int res, int arg, locint **ind_dom) {
	...
	{
	    free(temp_array);
	    temp_array = (locint *)  malloc(sizeof(locint) * 2*num);
	    max_arr_len = 2*num;
	}
	num1 = ind_dom[arg][0];
	if (max_arr_len1 < num1)
	{
	    free(temp_array1);
	    temp_array1 = (locint *)  malloc(sizeof(locint) * 2*num1);
	    max_arr_len1 = 2*num1;
	}
	...
	if (num1+num > ind_dom[res][1]-2)
	{
	  i = 2*(num1+num);
	    free(ind_dom[res]);
	    ind_dom[res] = (locint *)  malloc(sizeof(locint) * 2*i);
	    ind_dom[res][1] = 2*i;
	}
	i = 0;
        j = 0;
