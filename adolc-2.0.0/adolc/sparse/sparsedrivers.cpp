/*----------------------------------------------------------------------------
 ...
 Revision: $Id: sparsedrivers.cpp 276 2008-12-17 09:54:45Z awalther $
...
int sparse_jac(
	...
	return ret_val;
      ...
      *nnz = sJinfos.nnz_in;
      ...
      FILE *fp_JP;
      fp_JP = fopen("jac_pat.mtx","w");
	
      fprintf(fp_JP,"%d %d %d\n",depen,indep,*nnz);
      
      int ll = 0;
      for (int ii=0;ii<depen;ii++)
	{
	  for (int jj=1;jj<=(int) sJinfos.JP[ii][0];jj++)
	    {
	      fprintf(fp_JP,"%d %d %d\n",ii+1,sJinfos.JP[ii][jj]+1,ll);
	      ll++;
	      }
	}  
	fclose(fp_JP);
